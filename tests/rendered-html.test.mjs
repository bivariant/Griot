...
test("exports a branded static GitHub Pages site", async () => {
  ...
  assert.match(html, /Griot — Open Language Intelligence for Africa/);
  assert.match(html, /Open language intelligence for Africa/);
  assert.match(html, /Griot-MT/);
  assert.match(html, /Griot-ASR/);
  assert.match(html, /Griot is bigger than one model/);
